...
from app.llm.tools.code_executor import execute_code
from app.llm.tools.mcp_executor import execute_mcp_tool
...
async def execute_tool_call(
    ...
    if tool.type == "http":
        ...
        return await execute_mcp_tool(tool, arguments, timeout)
    else:
...
async def execute_http_tool(tool: Tool, arguments: dict, timeout: float = 30.0) -> str:
    ...
    return await shared_execute_http_tool(tool, arguments, timeout)


async def execute_code_tool(tool: Tool, arguments: dict, timeout: float = 60.0) -> str:
    """Execute a code tool in sandbox."""
    return await execute_code(tool, arguments, timeout)
