...
async def get_agent_stats(
    ...
    avg_duration = await msg_query.filter(
        role=MessageRole.ASSISTANT,
        duration_ms__isnull=False
    ).annotate(avg_duration=Avg("duration_ms")).values("avg_duration")
    
    avg_response_time = avg_duration[0]["avg_duration"] if avg_duration and avg_duration[0]["avg_duration"] else 0
    ...
    if granularity == "hour":
        ...
        for i in range(num_points):
            ...
            tokens = sum(
                ...
                for m in msgs_in_period if m["token_usage"]
            )
...
async def get_recent_conversations(
    ...
    conversations = await Conversation.filter(
        agent_id=agent_id
    ).order_by("-updated_at").limit(limit).prefetch_related("user")
