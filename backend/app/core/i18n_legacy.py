...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "builtin_tool_read_description": {
        "en": "Read a UTF-8 text file from the sandbox workspace. Use this to inspect files created earlier with the write tool or generated by Bash commands. Paths must stay inside /workspace. For binary files such as .docx, .xlsx, images, or archives, use bash commands like `ls -l`, `file`, or a Python script to inspect metadata instead of reading raw binary content.",
        "zh": "读取沙盒中的文本文件内容，让助手可以查看之前创建或生成的文件。",
    },
    "builtin_tool_read_param_max_chars_description": {
        "en": "Maximum characters to return from the text file. Increase this only when you need more content from a large file.",
        "zh": "文本文件返回的最大字符数。仅在需要读取大文件的更多内容时增加此值。",
    },
    ...
    "builtin_tool_write_description": {
        "en": "Write UTF-8 text content to a file inside the sandbox workspace. Use this before running non-trivial Python or Node code: write the script to /workspace/name.py or /workspace/name.js, then execute it with bash using `python3 /workspace/name.py` or `node /workspace/name.js`. Parent directories are created automatically. Use /workspace/... paths when calling this tool, but inside generated Python or Node scripts, prefer relative output paths such as `output/report.docx` or derive paths from the current working directory instead of hardcoding `/workspace/...`. For generated output files, keep them under /workspace, commonly /workspace/output/.",
        "zh": "在沙盒中创建或修改文本文件，常用于保存草稿、脚本或生成结果。",
    },
    "builtin_tool_write_param_content_description": {
        "en": "Complete UTF-8 text content to write. For scripts, include the full Python or JavaScript source code.",
        "zh": "要写入的完整 UTF-8 文本内容。对于脚本，包含完整的 Python 或 JavaScript 源代码。",
    },
    ...
    "fetch_webpage_unsupported_content_type": {
        "en": "Unsupported content type: {content_type}",
        "zh": "不支持的内容类型：{content_type}",
    },
