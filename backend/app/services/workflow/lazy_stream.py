...
class LazyStreamResult:
    ...
    async def execute(self, stream_to_node_id: str | None = None) -> str:
        ...
        logger.info(f"LazyStreamResult executing LLM for node {self.source_node_id}, streaming to {stream_to_node_id}")
        
        stream_manager = StreamManager(self.context.run_id) if stream_to_node_id else None
        ...
        async for chunk in model_manager.chat_stream(
            ...
            if chunk.delta and chunk.delta.content:
                ...
                if stream_manager and stream_to_node_id:
                    await stream_manager.publish_token(stream_to_node_id, chunk.delta.content)
        
        self._executed = True
        self._result = full_response
        
        logger.info(f"LazyStreamResult completed, response length: {len(full_response)}")
