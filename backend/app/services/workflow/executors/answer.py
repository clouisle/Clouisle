...
class AnswerNodeExecutor(NodeExecutor):
    ...
    async def execute(
        ...
        logger.info(
            f"Answer node {node_id}: is_streaming={is_streaming}, streaming_variable={streaming_variable}"
        )
        ...
        logger.info(
            f"Answer node {node_id}: answerConfig={answer_config}, output_vars={output_vars}"
        )
        ...
        if output_vars:
            for output_var in output_vars:
                ...
                if name and source_variable:
                    ...
                    should_stream_here = (
                        is_streaming
                        and streaming_variable
                        and source_variable == streaming_variable
                    )
                    ...
                    logger.info(
                        f"Answer node {node_id}: resolved '{source_variable}' -> {value}"
                    )
                    ...
                    if value is not None:
                        ...
                        logger.warning(
                            f"Answer node {node_id}: variable '{source_variable}' not found"
                        )
                else:
                    # sourceVariable is empty, skip this output with warning
                    logger.warning(
                        f"Answer node {node_id}: output '{name}' has no sourceVariable configured"
                    )

        # If no outputs from configured variables, try to get all upstream outputs automatically
        if not outputs:
            logger.info(
                f"Answer node {node_id}: no outputs resolved, collecting upstream outputs"
            )
                ...
                if out_node_id != node_id and isinstance(node_outputs, dict):
                    for var_name, value in node_outputs.items():
                        if value is not None:
                            ...
                            key = (
                                f"{out_node_id}.{var_name}"
                                if len(all_node_outputs) > 1
                                else var_name
                            )
                            outputs[key] = value
            logger.info(
                f"Answer node {node_id}: collected upstream outputs: {list(outputs.keys())}"
            )
