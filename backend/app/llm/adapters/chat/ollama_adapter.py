...
支持 Ollama 本地模型，包括 think 参数和 thinking 字段提取。
"""
...
class OllamaAdapter(BaseChatAdapter):
    """
    Ollama Chat 适配器

    特点：
    - OpenAI 兼容接口
    - 支持 think 参数（Boolean）：控制思考模型是否思考
    - 支持 thinking 字段提取：包含模型的思考过程
    - 本地部署，支持各种开源模型
    """
    ...
    def _convert_messages(self, messages: list[Message]) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            ...
            # Ollama 支持 thinking 字段
            if role_value == MessageRole.ASSISTANT.value:
                if msg.reasoning_content is not None:
                    msg_dict["thinking"] = msg.reasoning_content
