...
def _assert_read_error(content: bytes, message: str):
    with pytest.raises(BusinessError) as exc_info:
        ClouislePackageService._read_package("tool.clouisle", content)
...
def test_read_package_rejects_invalid_manifest(entries, message):
    _assert_read_error(_zip(entries), message)


@pytest.mark.parametrize(
    ("resource", "checksums", "message"),
    [
        (b"{}", b"not-json", "clouisle_checksum_mismatch"),
        (b"{}", b"{}", "clouisle_checksum_mismatch"),
        (b"not-json", None, "clouisle_missing_resource"),
        (b"[]", None, "clouisle_missing_resource"),
    ],
)
def test_read_package_rejects_invalid_metadata(resource, checksums, message):
    manifest = _manifest().model_dump(mode="json")
    manifest["checksums"] = {
        "resources/resource.json": clouisle_package._sha256(resource)
    }
    checksum_bytes = checksums or json.dumps(manifest["checksums"]).encode()
    _assert_read_error(
        _zip(
            {
...
def test_read_package_enforces_archive_limits(monkeypatch):
    package = ClouislePackageService.build_package(
    ...
    monkeypatch.setattr(clouisle_package, "_MAX_PACKAGE_BYTES", len(package) - 1)
    _assert_read_error(package, "clouisle_zip_too_large")
    monkeypatch.setattr(clouisle_package, "_MAX_PACKAGE_BYTES", len(package))
    monkeypatch.setattr(clouisle_package, "_MAX_FILE_COUNT", 1)
    _assert_read_error(package, "clouisle_invalid_zip")
    monkeypatch.setattr(clouisle_package, "_MAX_FILE_COUNT", 3)
    monkeypatch.setattr(clouisle_package, "_MAX_TOTAL_UNCOMPRESSED_BYTES", 1)
    _assert_read_error(package, "clouisle_zip_too_large")


def test_export_filename_and_app_version_fallback(monkeypatch):
    monkeypatch.setattr(
        clouisle_package, "now_utc", lambda: datetime(2026, 1, 2, 3, 4, 5, tzinfo=UTC)
    )
    assert (
        ClouislePackageService.export_filename(ClouisleResourceType.TOOL, " A__B! ")
        == "tool-a-b-20260102030405.clouisle"
    )
    monkeypatch.setattr("importlib.metadata.version", Mock(side_effect=RuntimeError))
    assert clouisle_package._app_version() == "0.1.0"
...
@pytest.mark.asyncio
async def test_install_without_membership_requires_existing_team(monkeypatch):
    session = _session()
    _patch_session_query(monkeypatch, session)
    monkeypatch.setattr(clouisle_package.Team, "filter", lambda **_kwargs: Query(None))
