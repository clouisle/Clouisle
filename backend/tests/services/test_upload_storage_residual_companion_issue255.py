...
@pytest.mark.anyio
async def test_local_upload_storage_reads_bytes_and_rejects_parent_traversal(
    tmp_path: Path,
):
    storage = LocalUploadStorage(tmp_path)
    await storage.save("safe/file.txt", b"payload")

    assert await storage.read("safe/file.txt") == b"payload"
    with pytest.raises(upload_storage.BusinessError) as exc:
        await storage.exists("../outside.txt")

    assert exc.value.status_code == 403


@pytest.mark.anyio
async def test_object_storage_settings_preserve_optional_defaults():
    storage = ObjectUploadStorage.from_settings(
        {
    ...
    assert storage.config.region is None
    assert storage.config.secure is False
    assert storage.config.force_path_style is False


@pytest.mark.anyio
async def test_object_storage_validate_translates_client_error(monkeypatch):
    error = ClientError(
        {"Error": {"Code": "AccessDenied"}, "ResponseMetadata": {}},
        "HeadBucket",
    )
    client = SimpleNamespace(head_bucket=AsyncMock(side_effect=error))
    ...
    monkeypatch.setattr(storage, "_client", client_context)

    with pytest.raises(RuntimeError, match="bucket validation failed") as exc:
...
@pytest.mark.anyio
async def test_object_storage_exists_reraises_non_missing_client_error(monkeypatch):
    error = ClientError(
        {
    ...
    with pytest.raises(ClientError) as exc:
        await storage.exists("private.txt")

    assert exc.value is error


@pytest.mark.anyio
async def test_object_storage_response_closes_client_when_get_fails(monkeypatch):
    exit_client = AsyncMock()

    class FailingClientContext:
        async def __aenter__(self):
            return SimpleNamespace(
                get_object=AsyncMock(side_effect=RuntimeError("down"))
            )

        async def __aexit__(self, exc_type, exc, tb):
            await exit_client(exc_type, exc, tb)
    ...
    monkeypatch.setattr(storage, "_client", FailingClientContext)

    with pytest.raises(RuntimeError, match="down"):
        await storage.response("file.txt")

    exit_client.assert_awaited_once_with(None, None, None)


@pytest.mark.anyio
async def test_object_storage_response_streams_fallback_type_and_closes_client(
    monkeypatch,
):
    body = SimpleNamespace(read=AsyncMock(side_effect=[b"first", b"second", b""]))
    exit_client = AsyncMock()

    class ClientContext:
        async def __aenter__(self):
            return SimpleNamespace(get_object=AsyncMock(return_value={"Body": body}))
    ...
    monkeypatch.setattr(storage, "_client", ClientContext)
