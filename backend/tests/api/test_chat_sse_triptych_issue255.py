...
@pytest.mark.anyio
async def test_chat_stream_persists_reasoning_content_and_usage(monkeypatch):
    agent = SimpleNamespace(
        id=uuid4(),
        team_id=uuid4(),
        team=SimpleNamespace(id=uuid4()),
    ...
    monkeypatch.setattr(chat.Conversation, "filter", Mock(return_value=update_query))
    monkeypatch.setattr(chat.Agent, "filter", Mock(return_value=update_query))
    monkeypatch.setattr(chat.Team, "filter", Mock(return_value=update_query))
...
@pytest.mark.anyio
async def test_edit_generator_persists_rag_reasoning_and_truncation(monkeypatch):
    user = SimpleNamespace(id=uuid4(), locale="en")
    agent = SimpleNamespace(
        id=uuid4(),
        team_id=uuid4(),
    ...
    edited = StoredMessage(conversation=conversation, role=MessageRole.USER)
    assistant = StoredMessage(conversation=conversation, role=MessageRole.ASSISTANT)
    ...
    monkeypatch.setattr(chat.Team, "filter", lambda **_kwargs: stats_query)
