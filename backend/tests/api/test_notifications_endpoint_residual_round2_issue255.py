...
@pytest.mark.asyncio
async def test_remaining_read_and_count_branches():
    current_user = user()
    visible = QueryStub(count=2, values=[uuid4()])
    with (
        patch.object(endpoint, "build_visible_query", AsyncMock(return_value=visible)),
        patch.object(
            endpoint.NotificationRead,
            "filter",
            Mock(return_value=QueryStub(values=[uuid4()])),
        ),
    ):
        result = await endpoint.get_unread_count(current_user=current_user)
    assert result["data"].total == 2
    assert any(call[0] == "exclude" for call in visible.calls)

    visible = QueryStub(values=[uuid4()])
    with (
        patch.object(endpoint, "build_visible_query", AsyncMock(return_value=visible)),
        patch.object(
            endpoint.NotificationRead,
            "filter",
            Mock(side_effect=[QueryStub(values=[uuid4()]), QueryStub(values=[])]),
        ),
        patch.object(endpoint.NotificationRead, "bulk_create", AsyncMock()),
        patch("app.models.notification.NotificationAudit.bulk_create", new=AsyncMock()),
    ):
        result = await endpoint.mark_read(
            NotificationReadRequest(mark_all=True), current_user=current_user
        )
...
@pytest.mark.asyncio
async def test_team_permission_superuser_and_missing_membership():
    team = SimpleNamespace(id=uuid4())
    with (
        patch.object(endpoint.Team, "filter", Mock(return_value=QueryStub(first=team))),
        patch.object(endpoint.TeamMember, "filter", Mock()) as memberships,
    ):
        assert (
            await endpoint.check_team_admin_permission(team.id, user(superuser=True))
            is team
        )
    ...
    with (
        patch.object(endpoint.Team, "filter", Mock(return_value=QueryStub(first=team))),
        patch.object(
            endpoint.TeamMember, "filter", Mock(return_value=QueryStub(first=None))
        ),
    ):
        with pytest.raises(BusinessError) as exc_info:
            await endpoint.check_team_admin_permission(team.id, user())
    assert_error(exc_info, ResponseCode.TEAM_ADMIN_REQUIRED, "team_admin_required")


@pytest.mark.asyncio
async def test_admin_list_team_access_filters_expiration_and_empty_deliveries():
    team_id = uuid4()
    query = QueryStub(count=0)
    with (
        patch.object(
            endpoint, "check_team_admin_permission", AsyncMock()
        ) as permission,
        patch.object(endpoint.Notification, "all", Mock(return_value=query)),
        patch.object(endpoint.NotificationDelivery, "filter", Mock()) as deliveries,
    ):
        result = await endpoint.admin_list_notifications(
    ...
    permission.assert_awaited_once_with(team_id, permission.await_args.args[1])
    deliveries.assert_not_called()
    assert result["data"]["items"] == []
    assert sum(call[0] == "filter" for call in query.calls) == 3


@pytest.mark.asyncio
async def test_admin_create_team_single_user_batch_and_invalid_scope():
    current_user = user(superuser=True)
    created = notification(scope=NotificationScope.TEAM, team_id=uuid4())
    with (
        patch.object(
            endpoint, "check_team_admin_permission", AsyncMock()
        ) as permission,
        ...
        result = await endpoint.admin_create_notification(
            payload(scope=NotificationScope.TEAM, team_id=created.team_id), current_user
        )
    permission.assert_awaited_once()
    assert result["data"].id == created.id

    target_id = uuid4()
    created = notification(scope=NotificationScope.USER, user_id=target_id)
    with (
        patch.object(
            endpoint.User,
            "filter",
            Mock(return_value=QueryStub(first=SimpleNamespace(id=target_id))),
        ),
        patch.object(endpoint.Notification, "create", AsyncMock(return_value=created)),
        patch.object(endpoint, "create_notification", AsyncMock()),
    ):
        result = await endpoint.admin_create_notification(
            payload(scope=NotificationScope.USER, user_id=target_id), current_user
        )
    assert result["data"].id == created.id

    target_ids = [uuid4(), uuid4()]
    created_rows = [
        notification(scope=NotificationScope.USER, user_id=target_id)
    ...
    with (
        patch.object(
            ...
            "filter",
            Mock(
                return_value=QueryStub(
                    rows=[SimpleNamespace(id=target_id) for target_id in target_ids]
                )
            ),
        ),
        patch.object(
            endpoint.Notification, "create", AsyncMock(side_effect=created_rows)
        ) as create,
        patch.object(endpoint, "create_notification", AsyncMock()) as publish,
    ):
        result = await endpoint.admin_create_notification(
            payload(scope=NotificationScope.USER, user_ids=target_ids), user()
        )
    assert create.await_count == publish.await_count == 2
    assert result["data"].id == created_rows[0].id

    invalid = payload().model_copy(update={"scope": "invalid"})
    with pytest.raises(BusinessError) as exc_info:
        await endpoint.admin_create_notification(invalid, current_user)
    assert_error(exc_info, ResponseCode.BAD_REQUEST, "invalid_notification_scope")
...
async def test_admin_create_rejects_incomplete_app_channel_config(
    channel, config_path, config
):
    with patch(config_path, new=AsyncMock(return_value=config)):
        with pytest.raises(BusinessError) as exc_info:
    ...
    assert_error(exc_info, ResponseCode.BAD_REQUEST, f"{channel.value}_not_configured")


@pytest.mark.asyncio
async def test_admin_delete_team_success_and_user_denial():
    team_item = notification(scope=NotificationScope.TEAM, team_id=uuid4())
    delete_query = QueryStub()
    with (
        patch.object(
            endpoint.Notification,
            "filter",
            Mock(side_effect=[QueryStub(first=team_item), delete_query]),
        ),
        patch.object(
            endpoint, "check_team_admin_permission", AsyncMock()
        ) as permission,
        patch.object(endpoint, "create_notification_audit", AsyncMock()),
    ):
        await endpoint.admin_delete_notification(team_item.id, user())
    permission.assert_awaited_once()

    user_item = notification(scope=NotificationScope.USER, user_id=uuid4())
    with patch.object(
        endpoint.Notification, "filter", Mock(return_value=QueryStub(first=user_item))
    ):
        with pytest.raises(BusinessError) as exc_info:
            await endpoint.admin_delete_notification(user_item.id, user())
    assert_error(
        exc_info, ResponseCode.INSUFFICIENT_PRIVILEGES, "insufficient_privileges"
    )


def test_serialize_delivery_error_translates_known_key():
    with (
        patch.object(endpoint, "has_translation", return_value=True),
        patch.object(endpoint, "t", return_value="translated") as translate,
    ):
        assert (
            endpoint.serialize_delivery_error(
                "notification_provider_error", NotificationDeliveryStatus.FAILED
            )
            == "translated"
        )
    translate.assert_called_once_with("notification_provider_error")
