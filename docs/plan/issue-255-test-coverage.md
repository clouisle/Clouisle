...
- Backend progress checkpoint (2026-07-19, commit `86f5d4d`): 835 tests passed and 2 were skipped; whole-app coverage is 40% after workflow executor and background-task behavior tests. The final 95% line and branch gates remain inactive.
- Frontend progress checkpoint (2026-07-19, commit `73a8c124`): 122 tests passed; Bun's loaded-module report is 49.41% lines and 37.14% functions after additional user/admin/API test batches. LCOV contains 72 sources and the census still fails because eligible sources are absent, so neither loaded-module percentages nor the coverage target have passed.
