...
- Frontend progress checkpoint (2026-07-19, commit `6532d4f8`): 65 tests passed; Bun's loaded-module report is 40.96% lines and 25.16% functions after chat/helper and API-client tests. The census still fails because eligible sources are absent from LCOV, so neither loaded-module percentages nor the coverage target have passed.
