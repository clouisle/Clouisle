...
- **Specific logic**:
  ...
  5. Keep ordinary text streaming independent from tool-call rendering; persist and rebuild each ordered segment without a text-markup question parser.
...
- **Validation**: Focused hook tests must prove two reasoning start/delta/end cycles, duplicate tool argument updates, terminal-state preservation, task placement, and exact part order both while streaming and after finalization/error/stop.
...
- **Specific logic**:
  ...
  5. Keep ordinary text, media, the durable `ask_user` form, truncation, and other visible parts in the same walk using existing renderers. Continue placing citation sources in the existing footer and stopped/error/action controls at message-level positions.
