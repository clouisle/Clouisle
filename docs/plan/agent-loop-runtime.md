...
- Production gaps found and fixed during migration: `app.tasks.agent.*` runs on the `default` queue (`main.py start_worker` consumes `default,knowledge,workflow`); `admin_observability.WORKER_QUEUES` includes `agent` (catalog + queue-length observability); `SANDBOX_WORKER_ENV_KEYS` includes `INTERNAL_API_TOKEN` (sandbox container needs it); sandbox env test made hermetic against `app.main` import-time `dotenv` pollution.
