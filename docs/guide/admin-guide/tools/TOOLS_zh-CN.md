...
| `rss_feed_reader` | 抓取并解析 RSS 或 Atom 订阅源，返回结构化文章列表 | 网页 |

### 记忆工具

启用 Agent 记忆后，模型可以使用用户记忆图的读写工具。关闭自动提取时，读取工具仍然可用；写入工具受 Agent 记忆配置控制。

| 工具 | 用途 | 限制与行为 |
|------|------|------|
| `search_memory` | 查找相关的用户记忆实体 | 支持 `top_k`（限制在 1-10）、可选时间窗口和实体类型筛选 |
| `get_memory_subgraph` | 读取搜索结果关联的关系 | 最多接受 5 个种子 ID 或精确名称，遍历深度为 1-3，可按 `incoming`、`outgoing` 或 `both`（默认 `both`）方向遍历，并可筛选关系类型 |

先使用 `search_memory` 找到候选实体；只有回答依赖实体关系时再使用 `get_memory_subgraph`。仅把返回的关系视为事实，不要推断结果中没有返回的边。
...
对已配置的数据库连接执行 SQL 查询（自定义工具类型 `database`）。

**功能特性：**
- 支持 PostgreSQL、MySQL、Redis 和 MongoDB 连接
- 主机/端口/数据库与凭证作为工具配置保存
- 每个工具可设置查询超时和最大返回记录数
...
| URL | 静态端点 URL（执行器会拒绝 `{{variable}}` 占位符，请改用查询参数、请求头或请求体传递变量） |
...
| 字段 | 描述 |
|------|------|
| 类型 | `postgresql`、`mysql`、`redis` 或 `mongodb` |
| 主机 / 端口 | 服务器地址（Redis/MongoDB 改用 `url`） |
| 数据库 | 数据库名称（Redis 用 `db` 序号，MongoDB 用 `auth_source`） |
| 用户名 / 密码 | 连接凭证 |
| SSL | TLS 配置 |
| 超时 | 查询超时时间（秒，默认 15，最大 120） |
| 最大条数 | 最大返回记录数（默认 100，最大 1000） |
