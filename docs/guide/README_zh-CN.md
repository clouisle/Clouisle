...
- **检索评估实验室**：内置检索命中测试与召回评估，方便对比评估分块策略与检索效果
- **可配置分块**：自定义分块大小、重叠和分隔符，支持预览与失败分块重试
- **重排序**：可选的重排序管道，提升检索准确度
- **嵌入管理**：可配置的嵌入模型和向量维度
- **异步处理**：通过 Celery 后台处理文档，支持状态追踪与批量操作
...
- **审计日志**：关键操作追踪，支持变更前后字段级差异快照和用户归属信息
- **管理后台可观测性**：集中查看平台用量、Token 消耗、延迟、错误率与系统监控
- **Clouisle 资源包**：标准化的 `.clouisle` 资产包，支持零明文凭据安全导出/导入与跨环境迁移（智能体、工作流、工具与知识库）
...
- **沙箱执行**：安全隔离的 Python/Node.js 代码执行环境，基于无特权（rootless）容器隔离与资源限制
