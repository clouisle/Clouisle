...
团队同样是平台内其他业务资源的隔离边界。新增资源类型时，应优先考虑：
...
- 团队 owner 不能直接离开团队，必须先转移所有权。
- 默认团队（`is_default=True`）不能删除。
- 未来新增业务资源应默认按团队隔离，而不是按全局资源处理。
...
运行时用量追踪围绕 `UsageTracker` 和 `ModelManager` 的团队级调用展开。

### 8.1 UsageTracker 能力

```python
await usage_tracker.check_quota(team_id="team-uuid", model_id="model-uuid", tokens_needed=1000)
await usage_tracker.record_usage(team_id="team-uuid", model_id="model-uuid", tokens_used=1500, request_count=1)
await usage_tracker.check_and_record_usage(team_id="team-uuid", model_id="model-uuid", tokens_used=1500)
stats = await usage_tracker.get_usage_stats(team_id="team-uuid", model_id="model-uuid")
```

### 8.2 团队级模型调用

`ModelManager` 提供带配额检查和用量记录的团队级方法：

```python
await model_manager.team_chat(team_id="team-uuid", messages=[...], model_id="openai/gpt-4o")
async for chunk in model_manager.team_chat_stream(team_id="team-uuid", messages=[...], model_id="openai/gpt-4o"):
    ...
await model_manager.team_embed(team_id="team-uuid", texts=["text1"], model_id="openai/text-embedding-3-small")
```

### 8.3 定时重置任务

- `tasks.reset_daily_usage`：每天 00:00 重置每日用量
- `tasks.reset_monthly_usage`：每月 1 日 00:05 重置每月用量

### 8.4 配额超限

当达到限制时会抛出 `QuotaExceededError`，常见类型包括：
- `daily_token`
- `monthly_token`
- `daily_request`
- `monthly_request`

前端应基于 `quota_type` 给出对应的提示。
