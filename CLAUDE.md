...
- Preferred types: `feat`, `fix`, `refactor`, `docs`, `test`, `chore`, `perf`, `ci`
- Commit messages must be **detailed and specific**, not vague
- The first line should clearly state the user-visible or architecture-relevant change
- When the change is non-trivial, add a body explaining the key decisions, scope, or why the change was needed
...
- Prefer messages like:
  - `fix: render generated media in assistant body instead of tool frames`
  - `feat: add media_result SSE event for chat media rendering`
- If one commit contains multiple tightly related changes, the subject should summarize the main outcome, and the body should list the important subchanges

Example:
```text
fix: render generated media in assistant body

Add a dedicated media_result SSE event for image and video outputs so the frontend can render media directly in the assistant body instead of inside tool result frames. Keep tool result payloads compact for LLM replay and update the API access page docs to describe the new event.
```
