...
const themeColors = {
  ...
  theme_sidebar_primary_color: '#0f172a',
  theme_sidebar_primary_foreground_color: '#f8fafc',
...
describe('theme config helpers', () => {
  ...
  test('maps every configured theme color to CSS variables', () => {
    ...
    expect(getBrandCssVariables(themeColors)).toMatchObject({
      '--primary': '#123456',
  ...
  test('falls back to core colors when optional sidebar and navbar colors are absent', () => {
