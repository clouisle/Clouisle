...
interface KnowledgeBaseShareDialogProps {
...
export function KnowledgeBaseShareDialog({
  knowledgeBase,
  open,
  onOpenChange,
  currentTeamId,
  availableTeams,
  onSuccess,
}: KnowledgeBaseShareDialogProps) {
  const t = useTranslations('platform.kb.share')
  const tCommon = useTranslations('common')

  const [shares, setShares] = React.useState<KnowledgeBaseShare[]>([])
  const [isLoading, setIsLoading] = React.useState(false)
  const [isSharing, setIsSharing] = React.useState(false)
  const [fieldErrors, setFieldErrors] = React.useState<Record<string, string>>({})
  const [selectedTeamId, setSelectedTeamId] = React.useState<string>('')
  const [selectedPermission] = React.useState<KnowledgeBaseSharePermission>('read_only')
  const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false)
  const [deletingShare, setDeletingShare] = React.useState<KnowledgeBaseShare | null>(null)
  ...
  }, [open, knowledgeBase?.id, loadShares])
  ...
  const selectedTeamName =
    availableTeamsToShare.find((team) => team.id === selectedTeamId)?.name || t('selectTeam')
  ...
  const handleShare = async () => {
    ...
    setIsSharing(true)
    try {
      await knowledgeBasesApi.shareKnowledgeBase(knowledgeBase.id, {
  ...
  // 打开取消共享确认对话框
  const handleDeleteClick = (share: KnowledgeBaseShare) => {
  ...
  const handleUnshare = async () => {
    if (!knowledgeBase?.id || !deletingShare) return
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('description', { kbName: knowledgeBase.name })}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-4 pb-4 border-b">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedPermission} disabled>
                      <SelectTrigger className="w-full">
                        <SelectValue>{t('permissions.readOnly')}</SelectValue>
                      </SelectTrigger>
                      <SelectContent alignItemWithTrigger={false}>
