...
mock.module('next/dynamic', () => ({
  default: () => function MarkdownPreview({ source }: { source: string }) {
    return { type: 'markdown-preview', props: { source } }
  },
}))
mock.module('@/lib/api', () => ({ knowledgeBasesApi: { getKnowledgeBase, search } }))
...
const knowledgeBase = {
  id: 'kb-1', name: 'Platform Handbook', settings: { rerank_enabled: true, rerank_candidate_k: 12, rerank_fail_open: false, rerank_score_threshold: 0.7 },
...
describe('platform SearchTestClient', () => {
  test('keeps loading visible until knowledge-base data resolves', async () => {
  ...
  test('validates search controls and renders reranked markdown results', async () => {
    ...
    find(tree, 'input', props => props.id === 'rerankCandidateK').props.onChange({ target: { value: '3' } })
    find(tree, 'input', props => props.id === 'rerankThreshold').props.onChange({ target: { value: '0.8' } })
    find(tree, 'input', props => props.id === 'rerankThreshold').props.onChange({ target: { value: '2' } })
    tree = render()
    ...
    expect(search).toHaveBeenCalledWith('kb-1', {
      ...
      rerank_enabled: true, rerank_candidate_k: 20, rerank_fail_open: false, rerank_score_threshold: 0.8,
    })
  ...
  test('does not search blank queries and recovers after an API failure', async () => {
    ...
    queryInput(tree).props.onChange({ target: { value: '   ' } })
    tree = render()
