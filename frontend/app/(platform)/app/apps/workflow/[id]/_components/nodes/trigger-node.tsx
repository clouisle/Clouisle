...
import { Zap, Type, AlignLeft, ListChecks, Hash, CheckSquare, File, Image, Files, Images } from 'lucide-react'
...
export function TriggerNode({ selected, data }: TriggerNodeProps) {
  ...
  return (
    <div className="group relative">
      ...
      <div className="flex items-center justify-between mb-2 px-1 h-5">
        <span className="text-xs text-muted-foreground">{t('nodesTrigger.startLabel')}</span>
      </div>
            ...
            {requiredParams.map((param) => {
              ...
              return (
                <div key={param.id} className="flex items-center justify-between bg-muted rounded-md px-2 py-1">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-muted-foreground">{t('nodesCommon.required')}</span>
