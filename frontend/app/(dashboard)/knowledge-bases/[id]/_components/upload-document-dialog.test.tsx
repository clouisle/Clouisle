...
mock.module('@/lib/api', () => ({ adminKnowledgeBasesApi: { uploadDocument } }))
...
mock.module('@/components/ui/dialog', () => ({
  Dialog: 'dialog', DialogContent: 'section', DialogDescription: 'p', DialogFooter: 'footer',
  DialogHeader: 'header', DialogTitle: 'h2',
}))
mock.module('@/components/ui/button', () => ({ Button: 'button' }))
mock.module('@/components/ui/progress', () => ({ Progress: 'progress' }))
mock.module('lucide-react', () => ({ Upload: 'upload-icon', FileText: 'file-icon', X: 'x-icon', Loader2: 'loader-icon' }))
...
describe('UploadDocumentDialog', () => {
  test('uploads valid files through the dashboard admin API and follows the success route', async () => {
  ...
  test('keeps controls locked during an upload and cleans loading state after API failure', async () => {
