      ...
      if (cachedDocs && cachedDocs.length === targetDocIds.length) {
        ...
        for (let i = 0; i < targetDocIds.length; i += batchSize) {
          ...
          const batchRes = await Promise.all(
            slice.map(id => adminKnowledgeBasesApi.getDocument(knowledgeBaseId, id).catch(() => null))
          )
          docs.push(...batchRes)
        }
      }
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push(`/knowledge-bases/${knowledgeBaseId}`)}>
