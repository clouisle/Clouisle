...
describe('DocumentsPreviewClient', () => {
  ...
  test('reports processing errors returned by the dashboard API', async () => {
    api.processDocumentWithChunks.mockResolvedValue({
      ...document,
      status: 'error',
    ...
    expect(toast.error).toHaveBeenCalledWith('invalid document')
    expect(push).toHaveBeenCalledWith('/knowledge-bases/kb-1')
  })
  ...
  test('validates process boundaries and supports chunk cancellation and empty-save deletion', async () => {
    let tree = await load()
    expect((button(tree, 'startProcessingAll (0)').props.disabled)).toBe(true)
    ...
    tree = render()
    expect(findAll(tree, node => node.type === 'textarea')).toHaveLength(0)

    ;(chunkAction(tree, 0).props.onClick as () => void)()
    tree = render()
    const textarea = findAll(tree, node => node.type === 'textarea')[0]
    ;(textarea.props.onChange as (event: { target: { value: string } }) => void)({ target: { value: '   ' } })
    tree = render()
    ;(button(tree, 'save').props.onClick as () => void)()
    tree = render()
    expect(text(tree)).not.toContain('Alpha beta')
  })

  test('updates all dashboard chunk settings and clears generated previews', async () => {
    let tree = await load()
    await (button(tree, 'previewChunks').props.onClick as () => Promise<void>)()
    tree = render()

    const overlap = findAll(tree, node => node.props.id === 'chunk_overlap')[0]
    ;(overlap.props.onChange as (event: { target: { value: string } }) => void)({ target: { value: '' } })
    tree = render()
    expect(findAll(tree, node => node.props.id === 'chunk_overlap')[0].props.value).toBe(0)

    const separator = findAll(tree, node => node.props.id === 'separator')[0]
    ;(separator.props.onChange as (event: { target: { value: string } }) => void)({ target: { value: '---' } })
    tree = render()
    expect(findAll(tree, node => node.props.id === 'separator')[0].props.value).toBe('---')

    const cleanText = findAll(tree, node => node.props.role === 'switch')[0]
    ;(cleanText.props.onClick as () => void)()
    tree = render()
    expect(findAll(tree, node => node.props.role === 'switch')[0].props['aria-checked']).toBe(false)
    expect(text(tree)).toContain('noPreviewYet')
  })

  test('removes tabs with mouse and keyboard using dashboard routes', async () => {
    api.getDocument.mockImplementation((_kb: string, id: string) => Promise.resolve({
      ...document,
      id,
      name: `${id}.pdf`,
    }))
    let tree = await load(['doc-1', 'doc-2'])
    let removes = findAll(tree, node => node.props.role === 'button' && node.type === 'span')
    ;(removes[0].props.onClick as (event: { stopPropagation(): void }) => void)({ stopPropagation() {} })
    tree = render(['doc-1', 'doc-2'])
    expect(text(tree)).not.toContain('doc-1.pdf')
    expect(text(tree)).toContain('doc-2.pdf')

    removes = findAll(tree, node => node.props.role === 'button' && node.type === 'span')
    ;(removes[0].props.onKeyDown as (event: { key: string; stopPropagation(): void }) => void)({ key: ' ', stopPropagation() {} })
    expect(push).toHaveBeenCalledWith('/knowledge-bases/kb-1')
  })
