...
import { adminAgentsApi, adminWorkflowsApi, teamsApi as adminTeamsApi, type AdminAgent, type Team } from '@/lib/api/admin'
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={agent.id} data-state={selectedAgents.has(agent.id) ? 'selected' : undefined}>
                ...
                <TableCell className="text-sm text-muted-foreground">{formatDateTime(agent.updated_at, locale)}</TableCell>
                <TableCell>
                  <DropdownMenu>
      ...
      {defaultTeamId && (
        <>
          ...
          <AppCreateDialog
            ...
            agentEditHref={(id) => `/apps/agents/${id}/edit`}
