...
export function APIKeysClient() {
    ...
    const loadAPIKeys = React.useCallback(async () => {
        ...
        try {
            const data = await apiKeysApi.getAPIKeys({
                page,
                pageSize,
                status: selectedStatuses.length > 0 ? selectedStatuses as Array<'active' | 'inactive' | 'expired'> : undefined,
    ...
    }, [page, pageSize, selectedStatuses, selectedUsers, debouncedSearchQuery])
    ...
    const handleUserFilterChange = (values: Set<string>) => {
